...
```{r install_Non-CRAN_libraries, eval=FALSE}
...
Plots showing the outlier, 
which was excluded from the final analysis
...
combo %>%
  select(3:6) %>% 
  gather(key = region_icc, value = Value, 1:2) %>% 
  gather(key = region_corr, value = Values, 1:2) %>% 
  summarise(med_icc = round(median(Value), 2), med_cor = round(median(Values),2), min_icc = round(min(Value),2), min_cor = round(min(Values),2), max_icc = round(max(Value),2), max_cor = round(max(Values),2)) %>% 
kable()
...
The next figure shows the relationship between kinfitr and pmod 
for the models that require arterial blood sampling
...
# Table 3 - consistency test_retest
...
```{r spag_2, fig.height = 15, fig.width = 10}
...
Correlation plots showing the relationship between the 
different models for the same Ligand.
...
```{r granville_cor, include = FALSE}
# cor_az <- macro_az %>% 
#   select(subjname, PETNo, region, Model, Value, software) %>% 
#   mutate(SoftMod = paste0(software, "_", region, "_", Model)) %>% 
#   select(-Model, -software, -region) %>% 
#   spread(SoftMod, Value)
# 
# azcor <- cor_az %>% 
#   select(kinfitR_OC_mrtm2:pmod_OC_srtm) %>% 
#   cor()
#   
# corrplot::corrplot.mixed(azcor, )
```
