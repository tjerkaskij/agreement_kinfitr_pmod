...
az <- az %>% 
  ...
  mutate(mrtm2 = map(mrtm2, ~select(.x, region, bp, se_bp))) %>% 
...
pk11195 <- pk11195 %>% 
  ...
  mutate(ma1 = map(ma1, ~select(.x, region, Vt, se_Vt))) %>% 
  mutate(logan = map(logan, ~select(.x, region, Vt, se_Vt))) %>%
  group_by(PET, subjname, PETNo) %>% 
...
# saveRDS(pmod_macro, 'Data_PMOD_RDS/pmod_macroparameters.rds')
